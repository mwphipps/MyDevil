...
{ 
  ...
  case EVENT_AI_SETSMOKEVIS: {
    int num = msg.ReadShort();
    int on = msg.ReadBits( 1 ) ? 1:0;
    Event_SetSmokeVisibility( num, on );
    return true;
    }
  };
...
void idAI::Killed( idEntity *inflictor, idEntity *attacker, int damage, const idVec3 &dir, int location ) {
    ...
    Event_BecomeRagdoll();
		StartSound( "snd_death", SND_CHANNEL_VOICE, 0, true, NULL );
	}
	...
	if ( spawnArgs.GetString( "model_death", "", &modelDeath ) ) {
		...
		StartSound( "snd_death", SND_CHANNEL_VOICE, 0, true, NULL );
    // Nicemice: added
    ProcessEvent( &EV_SetShaderParm, SHADERPARM_TIMEOFFSET, -MS2SEC( gameLocal.time ) );
    ProcessEvent( &EV_SetModel, idStr( modelDeath ) );
    //Event_SetShaderParm( SHADERPARM_TIMEOFFSET, -MS2SEC( gameLocal.time ) );
    //Event_SetModel( modelDeath );
...
// MyDevil Changed
	if ( ( attacker && attacker->IsType( idPlayer::Type ) ) && ( inflictor && !inflictor->IsType( idSoulCubeMissile::Type ) ) ) {
      static_cast< idPlayer* >( attacker )->AddAIKill( spawnArgs.GetInt( "exp", 0 ), spawnArgs.GetInt( "gold", 0 ) );
   }
}
