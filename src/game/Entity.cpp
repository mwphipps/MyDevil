...
void idEntity::ActivateTargets( idEntity *activator ) const {
  ...
  // Nicemice: we don't allow activating targets on the client
  if ( gameLocal.isClient ) {
    gameLocal.DWarning( "coop: idEntity::ActivateTargets called on client for %s(%s)", GetType()->classname, name.c_str() );
    return;
  }
	...
	for( i = 0; i < targets.Num(); i++ ) {
      ...
      // Nicemice: TEST DYNAMIC
      if ( ent->NetworkSyncTarget() ) {
        ent->DynamicNetworkSync( "target" );
      }
			ent->Signal( SIG_TRIGGER );
...
void idEntity::Event_BindToJoint( idEntity *master, const char *jointname, float orientated ) {
  //Nicemice: TEST DYNAMIC
  DynamicNetworkSync("bindToJoint");
...
void idEntity::Event_RemoveBinds( void ) {
  //Nicemice: TEST DYNAMIC
  DynamicNetworkSync("removeBinds");
	RemoveBinds();
}
...
void idEntity::Event_BindPosition( idEntity *master ) {
  //Nicemice: TEST DYNAMIC
  DynamicNetworkSync("bindPosition");
	Bind( master, false );
}
...
/*
================
idEntity::Event_SetModel - Nicemice: modified
================
*/
...
void idEntity::Event_SetSkin( const char *skinname ) {
  ...
  if ( gameLocal.isServer ) {
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idEntity::Event_SetSkin unhandled");
  }
...
/*
================
idEntity::Event_SetShaderParm - Nicemice: modified
================
*/
...
void idEntity::Event_SetShaderParms( float parm0, float parm1, float parm2, float parm3 ) {
  if ( gameLocal.isServer ) {
    //Nicemice: Needs to be fixed?!
    gameLocal.Warning("coop: idEntity::Event_SetShaderParms unhandled");
  }
...
/*
================
idEntity::NetworkSyncTarget - Nicemice: added
================
*/
bool idEntity::NetworkSyncTarget() const {
  return false;
}
...
void idEntity::ServerSendEvent( int eventId, const idBitMsg *msg, bool saveEvent, int excludeClient ) {
  ...
  if ( !fl.networkSync && !isMapEntity ) {
    // it is a temporary entity and we don't have networksync on
    // it won't exist on the client so we don't send any events
    return;
  }
...
/*
================
idEntity::DynamicNetworkSync - Nicemice: added
================
*/
void idEntity::DynamicNetworkSync(const char* reason ) {
  
  if ( fl.networkSync ) {
    return;
  } else if ( isMapEntity ) {
    gameLocal.DPrintf( "coop: Adding networksync(%s) %s(%s)\n", reason, GetType()->classname, name.c_str() );
    fl.networkSync = true;
  }
}
...
/*
================
idAnimatedEntity::NetworkSyncTarget - Nicemice: added
================
*/
bool idAnimatedEntity::NetworkSyncTarget() const {
  return true;
};
