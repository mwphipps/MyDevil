...
void idInventory::RestoreInventory( idPlayer *owner, const idDict &dict ) {
	...
	// health/armor
	maxHealth		= owner->GetLevel()*5 + dict.GetInt( "maxhealth", "100" ); // SnoopJeDi - Mydevil
...
idPlayer::idPlayer() {
	...
	selfSmooth				= false;
	inventory.exp		    = 0; // Mydevil - SnoopJeDi Added
	inventory.gold          = 0; // MyDevil Added
	nextClass               = ""; // MyDevil - SillyRabbit  
	level					= 0; // SnoopJeDi
}
...
void idPlayer::Init( void ) {
	...
	nextClass = "";               // MyDevil - Silly Rabbit
}
...
// MyDevil Added. NiceMice points out... this should be saved, not restored to carry over mapchange
    playerInfo.SetInt( "exp", inventory.exp );
    playerInfo.SetInt( "gold", inventory.gold );	
}
...
void idPlayer::RestorePersistantInfo( void ) {
	...
	gameLocal.Printf( "^1Exp is %i", inventory.exp ); // MyDevil SnoopJedi Debug
	if ( gameLocal.isMultiplayer ) {
...
bool idPlayer::HandleSingleGuiCommand( idEntity *entityGui, idLexer *src ) {
	...
	if ( token.Icmp( "buy" ) == 0 ) { //SnoopJeDi
			idUserInterface *gui = entityGui->GetRenderEntity()->gui[0];
			const char *type;
			switch( gui->GetStateInt( "selected" ) ) {
				case 0: 
					type = "weapon";
					break;
				case 1:
				...
				case 4:
					...
					break;
				default:
					gameLocal.Warning( "Couldn't get gui::selected on shop\n" );
					break;
			}

			const char *n = gui->GetStateString( va( "%sList_sel_0", type ) );
			const idStr temp = gui->GetStateString( va( va( "%sList_item_%s", type ), n ) );
			const idStr index = gui->GetStateString( va( va( "%sIndex_item_%s", type ), n ) );
			const idDeclEntityDef *def = gameLocal.FindEntityDefByIndex( atoi( index ), true );
			int cost = def->dict.GetInt( "cost", "100" );
			if (inventory.gold >= cost) {
				//gameLocal.Printf("^1Player has %i gold, buy accepted\n", inventory.gold);			
				inventory.gold -= cost;
				GiveItem( def->GetName() );
				gui->HandleNamedEvent( "buyAccept" );
			} else {
				gui->HandleNamedEvent( "buyRefuse" );
				//gameLocal.Printf("^1Player has %i gold, buy refused\n", inventory.gold);			
			}
				
			//gameLocal.Printf("^1Weapon is %s\n", wepdef->GetName() );
			//gameLocal.Printf( va( "^1Cost of weapon is %i\n", wepdef->dict.GetInt( "cost", "100" ) ) );
			return true;
	}

#if 0 // SnoopJeDi
	if ( token.Icmp( "sell" ) == 0 ) {
		// I'm not really doing work here, but it's for later
		idUserInterface *gui = entityGui->GetRenderEntity()->gui[0];
		const char *n = gui->GetStateString( "sellList_sel_0", type );
		const idStr temp = gui->GetStateString( "sellList_item_%s", n );
		const idStr index = gui->GetStateString( "sellIndex_item_%s", n );
		const idDeclEntityDef *def = gameLocal.FindEntityDefByIndex( atoi( index ), true );
	}
#endif
...
// Mydevil - Jizaboz: Let's leave the scoreboard in for now...
...
*/
// MyDevil Changed
void idPlayer::AddAIKill( int addexp, int addgold ) {
   int max_souls;
   int ammo_souls;

// MyDevil Added
   inventory.exp += addexp; 
...
=============
*/
void idPlayer::CheckExp( void ) {
		int expneeded;

		gameLocal.Printf( "^1In CheckExp(), level is %i\n", level );
		expneeded = spawnArgs.GetInt( va( "level_%i", level + 1 ), "-1" );
		gameLocal.Printf( "^1In CheckExp(), expneeded is %i\n", expneeded );
		if ( expneeded != -1 && inventory.exp >= expneeded ) {
			level++;
			inventory.maxHealth = level*5 + spawnArgs.GetInt( "maxhealth", "100" );
			gameLocal.Printf( "^1Level incremented!\n" );
		}
}
...
*/
	...
	if ( hud ) {
		if ( MPAim != -1 ) {
// MyDevil (SnoopJedi) moved here to eliminate hit sound when shooting monsters			
...
// Mydevil - Jizaboz: Let's leave the scoreboard for now...
	scoreBoardOpen = ( ( usercmd.buttons & BUTTON_SCORES ) != 0 || forceScoreBoard );
...
void idPlayer::WriteToSnapshot( idBitMsgDelta &msg ) const {
	...
	msg.WriteShort( inventory.exp ); // Mydevil Added
	msg.WriteShort( inventory.gold ); // Mydevil Added
...
void idPlayer::ReadFromSnapshot( const idBitMsgDelta &msg ) {
	...
	inventory.exp = msg.ReadShort();    // MyDevil Added
	inventory.gold = msg.ReadShort();    // MyDevil Added
...
/*
================
idPlayer::CanChangeClassTo
  returns true iff the player can legally change to this class
  things to check in this method are:
  is there an entityDef for that class?
  is the class on the same team as the player? (for team based games)
  any other checks you could think up...
================
*/
bool idPlayer::CanChangeClassTo( const char* classname ) {
        // sanity checks
        assert( classname != NULL );

        if ( !gameLocal.isMultiplayer )
                // at the moment, the multiclass mechanism is only suitable for multiplayer
                return false;

        // does the class exist?
        const idDeclEntityDef *def = gameLocal.FindEntityDef( classname, false );
        if ( !def ) {
                // this shouldn't happen but can if the changeClass SysCmd is used
                // and the client enters a bogus or misspelt classname
                gameLocal.Warning( "Can't change to class '%s', unknown classname.\n", classname );
                return false;
        }

        if ( gameLocal.gameType == GAME_TDM ) { // or any other team based game
                // game is team based, check that the class is on the same team
                int newTeam;
                newTeam = def->dict.GetInt( "team", "-1" );
                if ( newTeam >= 0 && newTeam != team )
                        // class belongs to the other team
                        return false;
        }
...
void idPlayer::SetNextClass (const char* classname) {
        nextClass = classname;
}

char const* idPlayer::GetNextClass (void) {
        const char* ret = nextClass.c_str();
        if ( !ret || !*ret )
                // if no next class is set, return the current classname
                ret = spawnArgs.GetString( "classname" );
        return ret;
} 
