...
// NeedsClassChange, is set to true if this player needs to be respawned as new class
    bool            needsClassChange;	
} mpPlayerState_t;
...
// MyDevil Added (Silly Rabbit player Classes)	
    idUserInterface *classmenuGui;          // MULTICLASS MENU 
...
private:
    ...
    // MyDevil Added (Silly Rabbit Player Classes)
public: 
    static void     ChangeClassRequest_f( const idCmdArgs &args ); 
    void            ChangeClassRequest( const char *classname );
    bool            ChangeClass( int clientNum, const char *classname ); 
    bool            NeedsClassChange( int clientNum );
    bool			ChangeClassNow( int clientNum );
    
    idPlayer *      Respawn( idPlayer *p );
   
    static void             StartClassMenu_f( const idCmdArgs &args );
    void                    StartClassMenu( void );
	void					SetUpShops( void ); // SnoopJeDi
