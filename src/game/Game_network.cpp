...
// Nicemice: changed
idCVar net_clientShowSnapshotRadius( "net_clientShowSnapshotRadius", "1024", CVAR_GAME | CVAR_FLOAT, "" );
...
// MyDevil Added - Silly Rabbit Player Classes
    const char *classname = entities[ clientNum ]->spawnArgs.GetString( "classname" ); 
...
// MyDevil Added (Silly Rabbit Player Classes)
        const char *classname = entities[ i ]->spawnArgs.GetString( "classname" ); 
        outMsg.WriteString( classname ); 		
		networkSystem->ServerSendReliableMessage( clientNum, outMsg );
	}
...
// MyDevil Added (Silly Rabbit Player Classes)
        case GAME_RELIABLE_MESSAGE_CHANGECLASS: { 		
		     char classname[128]; // is 128 a good limit?
		     msg.ReadString( classname, sizeof( classname ) ); 
		     mpGame.ChangeClass( clientNum, classname );
		     break; 
	    }     
		default: {
...
/*
================
idGameLocal::ClientShowSnapshot - Nicemice: modified
================
*/
void idGameLocal::ClientShowSnapshot( int clientNum ) const {
  ...
  int all = 0;
    ...
    const bool isTransmitted = ( baseBits != 0 );
    const bool isInSnap = ( ent->snapshotBits != 0 );

    idVec4 color = colorMdGrey;
    all++;
    ...
    if ( !isInSnap && net_clientShowSnapshot.GetInteger() == 1 ) {
      // we only want to draw updated entities
			continue;
    }

    if ( !isTransmitted && net_clientShowSnapshot.GetInteger() == 2 ) {
      // we only want to draw updated and currently not transmitted (but networksynced) entities
      continue;
    }
		...
		gameRenderWorld->DebugBounds( color, entBounds );
    if ( isInSnap ) {
		  gameRenderWorld->DrawText( va( "%d: %s (%d,%d bytes of %d,%d)\n", ent->entityNumber,
						  ent->name.c_str(), ent->snapshotBits >> 3, ent->snapshotBits & 7, baseBits >> 3, baseBits & 7 ),
							  entBounds.GetCenter(), 0.2f, colorWhite, viewAxis, 1 );
    }
	}

  idStr text;
  sprintf( text, "snapshot entities = %d/%d/%d", snap, trans, all );
	...
	for( ent = spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) {
		...
		if ( !( snapshot->pvs[ent->entityNumber >> 5] & ( 1 << ( ent->entityNumber & 31 ) ) ) ) {
          ...
          if ( clientEntityStates[clientNum][ent->entityNumber] ) {
            ...
            if ( net_clientShowSnapshot.GetInteger() ) {
              // we only want to draw updated entities
              gameRenderWorld->DebugBounds( colorRed, ent->GetPhysics()->GetAbsBounds() );
              idStr staleent;
              sprintf( staleent, "stale: %s(%s)", ent->GetType()->classname, ent->name.c_str() );
              gameRenderWorld->DrawText( staleent.c_str(), ent->GetPhysics()->GetAbsBounds().GetCenter(), 0.2f, colorWhite, gameLocal.GetLocalPlayer()->viewAngles.ToMat3(), 1 );
		        }
            gameLocal.DWarning( "map entity %s(%s) is stale", ent->GetType()->classname, ent->name.c_str() );
            continue;
          }
				} else {
					ent->FreeModelDef();
...
// MyDevil Replaced SpawnID (Silly Rabbit Player Classes)
			char classname[MAX_STRING_CHARS];
			msg.ReadString( classname, sizeof( classname ) );
			if ( !entities[ client ] ) { 
				SpawnPlayerAs( client, classname ); 
