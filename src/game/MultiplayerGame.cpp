...
const char *idMultiplayerGame::MPGuis[] = {
	...
	"guis/mpmsgmode.gui",
	"guis/netmenu.gui",
// MyDevil Added ( Silly Rabbit Player Classes )
    "guis/classmenu.gui", // MULTICLASS MENU 	
	NULL
};
...
idMultiplayerGame::idMultiplayerGame() {
	...
	msgmodeGui = NULL;
// MyDevil Added ( Silly Rabbit PLayer Classes )
    classmenuGui = NULL; // MULTICLASS MENU	
...
// MyDevil Added ( Silly Rabbit Player Classes )
    classmenuGui = uiManager->FindGui( "guis/classmenu.gui", true, false, true );
    classmenuGui->SetStateBool( "gameDraw", true );
// MyDevil Added (5/28/06) SnoopJeDi
...
// MyDevil Added (Silly Rabbit player classes)
    if ( !gameLocal.isClient && playerState[ clientNum ].needsClassChange )
	{
    	playerState[ clientNum ].needsClassChange = false; 
    	return;
    }
...
void idMultiplayerGame::Clear() {
	...
	msgmodeGui = NULL;
// MyDevil Added
    classmenuGui = NULL; // Initialize our new menu gui 	
	if ( mapList ) {
...
idUserInterface* idMultiplayerGame::StartMenu( void ) {
	...
	if ( currentMenu == 1 ) {
		...
		return msgmodeGui;
// MyDevil Added ( Silly Rabbit Player Classes)
    } else if ( currentMenu == 4 ) { // MULTICLASS MENU 
    	classmenuGui->Activate( true, gameLocal.time );
    	cvarSystem->SetCVarBool( "ui_chat", true );
    	return classmenuGui;
...
// MyDevil Added ( Silly Rabbit Player Classes )
	    ...
	    // don't forget to also deactivate our new menu 
	    classmenuGui->Activate( false, gameLocal.time );
...
const char* idMultiplayerGame::HandleGuiCommands( const char *_menuCommand ) {
	...
	if ( currentMenu == 1 ) {
		currentGui = mainGui;
// MyDevil modified ( Silly Rabbit Player Classes)
    } else if ( currentMenu == 2) {
	    currentGui = msgmodeGui; 
	} else { // assume currentMenu == 4
	    currentGui = classmenuGui;     
	}
	...
	for( icmd = 0; icmd < args.Argc(); ) {
		...
		if ( !idStr::Icmp( cmd,	";"	) )	{
			...
			return _menuCommand;
// MyDevil Added ( Silly Rabbit Player Classes)
        } else if ( !idStr::Icmp( cmd, "changeclass" ) ) { 
	        // handle the 'changeclass <classname>' gui command
	        if ( gameLocal.isMultiplayer && gameLocal.GetLocalPlayer() ) { 
		        idStr classname = args.Argv( icmd++ );
		        gameLocal.mpGame.ChangeClassRequest( classname.c_str() ); 
	        }     
			continue;
		} 
...
// MyDevil Modified ( Silly Rabbit Player Classes )
            ...
            classmenuGui->Redraw( gameLocal.time ); 
			}
...
MyDevil p->ServerSpectate( false ) replaced with p = Respawn( p ) for Player Classes 
================
*/
...
/*
================
idMultiplayerGame::ChangeClassRequest_f (CLIENT/SERVER)
  handler for the 'changeClass' SysCmd
  usage: changeClass <classname>
================
*/
void idMultiplayerGame::ChangeClassRequest_f( const idCmdArgs &args ) {
        idPlayer *p = gameLocal.GetLocalPlayer ();
        if ( !p ) {
                gameLocal.Warning( "no local player\n" );
                return;
        }
        if ( args.Argc() < 2 ) {
                gameLocal.Printf( "Your current class is '%s'.  Your next class will be '%s'\n",
                                                  p->spawnArgs.GetString( "classname" ),
                                                  p->GetNextClass () );
                return;
        }
        const char* classname = args.Argv(1);
        gameLocal.mpGame.ChangeClassRequest( classname );
}

/*
=================
idMultiplayerGame::ChangeClassRequest (CLIENT/SERVER)
  sends a reliable message to the server asking it to change the
  class of the local player to the given classname on the next spawn
=================
*/
void idMultiplayerGame::ChangeClassRequest( const char* classname ) {
        // sanity checks
        assert ( classname != NULL );          // classname must not be null
        assert ( gameLocal.GetLocalPlayer() ); // make sure there is a local player

        if ( !gameLocal.isClient ) {

                // we are the server, so we can't send messages to ourselves
                ChangeClass( gameLocal.localClientNum, classname ); // don't care what it returns
        } else {
                // remember the class we asked for
                idPlayer *player = gameLocal.GetLocalPlayer ();
                player->SetNextClass( classname );
...
/*
================
idMultiplayerGame::ChangeClass (CLIENT/SERVER)
  checks whether the client is allowed to change to this class,
  then if so, records what the next class for that player will
  be and sends feedback back to the client
    called by ChangeClassRequest, *ProcessReliableMessages
================
*/
bool idMultiplayerGame::ChangeClass( int clientNum, const char* classname ) {
        bool changed = false;
        idPlayer *player;
        player = static_cast<idPlayer *>( gameLocal.entities[ clientNum ] );
        // quick sanity check
        assert ( player->IsType( idPlayer::Type ) );
        if ( gameLocal.isClient ) {
                assert ( clientNum == gameLocal.localClientNum );
                // we got called by ClientProcessReliableMessages
                // 'classname' is the name of our next class according to the server
                if ( idStr::Icmp( classname, player->GetNextClass() ) ) {
                        // helpful feedback.  this can be removed or changed to some gui feedback system
                        gameLocal.Printf( "Class change to '%s' denied by the server\n", player->GetNextClass() );
                        player->SetNextClass( classname );
                } else {
                        gameLocal.Printf( "Class change to '%s' accepted by the server\n", player->GetNextClass() );
                        changed = true;
                }
                // feedback
        } else { // !gameLocal.isClient
                // we got called by ServerProcessReliableMessages or ChangeClassRequest
                // let the player object to all the checks...
                if ( player->CanChangeClassTo( classname ) ) {
                        player->SetNextClass( classname );
                        changed = true;
                }
                if ( clientNum == gameLocal.localClientNum ) {
                        gameLocal.Printf( "Class change to '%s' %s\n", classname, ( changed ? "accepted" : "denied" ) );
                } else {
                        // feedback message
                        idBitMsg outMsg;
                        byte     msgBuf[ MAX_GAME_MESSAGE_SIZE ];
        ...
        playerState[ clientNum ].needsClassChange = changed;
//   added this bit of code which suicides the player after      
//   a successfull class change so that the player canspawn 
//   as the new class. 
     if ( changed && clientNum == gameLocal.localClientNum ) { 
             if ( gameLocal.isClient ) {
	                 idBitMsg        outMsg; 
	                 byte            msgBuf[ MAX_GAME_MESSAGE_SIZE ];
	                 outMsg.Init( msgBuf, sizeof( msgBuf ) ); 
...
/*
===============
idMultiplayerGame::NeedsClassChange (CLIENT/SERVER)
===============
*/
bool idMultiplayerGame::NeedsClassChange( int clientNum ) {
        // clients must only call this method on the local client
        assert ( !gameLocal.isClient || clientNum == gameLocal.localClientNum );
        idPlayer *player = static_cast<idPlayer *>( gameLocal.entities[ clientNum ] );
        const char *currentClass = player->spawnArgs.GetString( "classname" );
        const char *nextClass = player->GetNextClass();
        // returns true of the needsClassChange flag as been set
        // and the next class is actually different from the current class of the player
        return ( playerState[ clientNum ].needsClassChange &&
                         nextClass != 0 &&
                         *nextClass != '\0' &&
                         idStr::Icmp( currentClass, nextClass ) );
}

/*
===============
idMultiplayerGame::ChangeClassNow (SERVER ONLY)
  perhaps named misleadingly, ChangeClass() does not actually change the class
  of the player, just stores the name of the next class for when the player dies
  and is respawned.
  ChangeClassNow does the actual change, and should only be called when the
  player is dead or spectating.
===============
*/
bool idMultiplayerGame::ChangeClassNow( int clientNum ) {
        // make sure it's server only
        assert( !gameLocal.isClient );

        idPlayer *player = static_cast<idPlayer *>( gameLocal.entities[ clientNum ] );

        // remember a few things from the player entity
        // that need to be carried over to the new entity
        // there's probably other stuff that needs to be carried over
        // but I can't think of them now.
        int oldTeam        = player->team;
        int oldSpawnedTime = player->spawnedTime;
        int oldTourneyRank = player->tourneyRank;

        // actually try the change now
        // ChangeClassNow sets player to point to the new player entity on success
        if ( gameLocal.ChangeClassNow( clientNum, player->GetNextClass() ) ) {
                // get a pointer to the new player entity
                player = static_cast<idPlayer *>( gameLocal.entities[ clientNum ] );
                ...
                // clean up
                player->team        = oldTeam;
                player->spawnedTime = oldSpawnedTime;
                player->tourneyRank = oldTourneyRank;
                return true;
        }
...
// helper.  call this instead of p->ServerSpectate( false )
idPlayer *idMultiplayerGame::Respawn( idPlayer *p ) {
        //   we are going to spawn this player, so change his class if we have to
        int clientNum = p->entityNumber;
        if ( NeedsClassChange( clientNum ) &&
            ChangeClassNow( clientNum ) )
                // p is nolonger valid.  get pointer to the new player entity for this client
                p = gameLocal.GetClientByNum( clientNum );
        p->ServerSpectate( false );
        return p;
} 


 // <SillyRabbit> MULTICLASS MENU
 void idMultiplayerGame::StartClassMenu_f( const idCmdArgs &args ) {
        if ( gameLocal.isMultiplayer && 0 != gameLocal.GetLocalPlayer() )
                gameLocal.mpGame.StartClassMenu();
 }
 
 void idMultiplayerGame::StartClassMenu( void ) {
        nextMenu = 4;
        gameLocal.sessionCommand = "game_startmenu";
 }

 void idMultiplayerGame::SetUpShops( void ) { // SnoopJeDi
	idStr guiName;
	int maxentitydefs = declManager->GetNumDecls( DECL_ENTITYDEF );
	const idDeclEntityDef *temp;
	for ( idEntity * ent = gameLocal.spawnedEntities.Next(); ent != NULL; ent = ent->spawnNode.Next() ) 
		if ( ent->GetRenderEntity()->gui[ 0 ] ) {
			idUserInterface *gui = ent->GetRenderEntity()->gui[ 0 ];  // FIXME: Loop through all guis?
				if ( gui ) {
					guiName = gui->Name();
					//gameLocal.Printf( va("^1Gui with name %s\n", guiName.c_str() ) );
					if ( guiName == "guis/shop.gui" ) {
						//gameLocal.Printf( va("^1Shop.gui on %s(%i) of level %i\n", ent->GetName(), i, gui->GetStateInt( "gui_parm1", "0" ) ) );
						ent->fl.networkSync = true;
						int c = 0;
						int guiLevel = gui->GetStateInt( "gui_parm1", "0" );
						for ( int j = 0; j < maxentitydefs; j++ ) {
							temp = gameLocal.FindEntityDefByIndex( j, false );
							//gameLocal.Printf("^1Trying %s\n", temp->GetName() );
							if ( temp->GetName() && !idStr::Icmpn( temp->GetName(), "weapon_", 7 ) ) {
								//gameLocal.Printf("^1Adding %s\n", temp->GetName() );
								temp = gameLocal.FindEntityDefByIndex( j, true );
								if ( temp->dict.GetBool( "buyable", "0" ) && temp->dict.GetInt( "minlevel", "0" ) <= guiLevel && temp->dict.GetInt( "maxlevel", "999" ) >= guiLevel ) {
									gui->SetStateString( va( "weaponList_item_%i", c ), va( "%s\t%i", temp->dict.GetString( "shop_name", temp->GetName() ), temp->dict.GetInt( "cost", "100" ) ) );
									gui->SetStateString( va( "weaponIndex_item_%i", c), va( "%i", temp->Index() ) );
									//gameLocal.Printf( "^1Added %s\n", temp->GetName() );
									c++;
									temp = NULL;
								}
							}							
						...
						for ( int j = 0; j < maxentitydefs; j++ ) {
							temp = gameLocal.FindEntityDefByIndex( j, false );
							//gameLocal.Printf("^1Trying %s\n", temp->GetName() );
							if ( temp->GetName() && !idStr::Icmpn( temp->GetName(), "ammo_", 5 ) ) {
								//gameLocal.Printf("^1Adding %s\n", temp->GetName() );
								temp = gameLocal.FindEntityDefByIndex( j, true );
								if ( temp->dict.GetBool( "buyable", "0" ) && temp->dict.GetInt( "minlevel", "0" ) <= guiLevel && temp->dict.GetInt( "maxlevel", "999" ) >= guiLevel ) {
									gui->SetStateString( va( "ammoList_item_%i", c ), va( "%s\t%i", temp->dict.GetString( "shop_name", temp->GetName() ), temp->dict.GetInt( "cost", "100" ) ) );
									gui->SetStateString( va( "ammoIndex_item_%i", c), va( "%i", temp->Index() ) );
									c++;
									temp = NULL;
								}
							}
						...
						for ( int j = 0; j < maxentitydefs; j++ ) {
							temp = gameLocal.FindEntityDefByIndex( j, false );
							//gameLocal.Printf("^1Trying %s\n", temp->GetName() );
							if ( temp->GetName() && !idStr::Icmpn( temp->GetName(), "potion_", 7 ) ) {
								//gameLocal.Printf("^1Adding %s\n", temp->GetName() );
								temp = gameLocal.FindEntityDefByIndex( j, true );
								if ( temp->dict.GetBool( "buyable", "0" ) && temp->dict.GetInt( "minlevel", "0" ) <= guiLevel && temp->dict.GetInt( "maxlevel", "999" ) >= guiLevel ) {
									gui->SetStateString( va( "potionsList_item_%i", c ), va( "%s\t%i", temp->dict.GetString( "shop_name", temp->GetName() ), temp->dict.GetInt( "cost", "100" ) ) );
									gui->SetStateString( va( "potionsIndex_item_%i", c), va( "%i", temp->Index() ) );
									c++;
									temp = NULL;
								}
							}
						...
						for ( int j = 0; j < maxentitydefs; j++ ) {
							temp = gameLocal.FindEntityDefByIndex( j, false );
							//gameLocal.Printf("^1Trying %s\n", temp->GetName() );
							if ( temp->GetName() && !idStr::Icmpn( temp->GetName(), "scroll_", 7 ) ) {
								//gameLocal.Printf("^1Adding %s\n", temp->GetName() );
								temp = gameLocal.FindEntityDefByIndex( j, true );
								if ( temp->dict.GetBool( "buyable", "0" ) && temp->dict.GetInt( "minlevel", "0" ) <= guiLevel && temp->dict.GetInt( "maxlevel", "999" ) >= guiLevel ) {
									gui->SetStateString( va( "scrollsList_item_%i", c ), va( "%s\t%i", temp->dict.GetString( "shop_name", temp->GetName() ), temp->dict.GetInt( "cost", "100" ) ) );
									gui->SetStateString( va( "scrollsIndex_item_%i", c), va( "%i", temp->Index() ) );
									c++;
									temp = NULL;
								}
							}
						...
						for ( int j = 0; j < maxentitydefs; j++ ) {
							temp = gameLocal.FindEntityDefByIndex( j, false );
							//gameLocal.Printf("^1Trying %s\n", temp->GetName() );
							if ( temp->GetName() && !idStr::Icmpn( temp->GetName(), "shopitem_", 7 ) ) {
								//gameLocal.Printf("^1Adding %s\n", temp->GetName() );
								temp = gameLocal.FindEntityDefByIndex( j, true );
								if ( temp->dict.GetBool( "buyable", "0" ) && temp->dict.GetInt( "minlevel", "0" ) <= guiLevel && temp->dict.GetInt( "maxlevel", "999" ) >= guiLevel ) {
									gui->SetStateString( va( "otherList_item_%i", c ), va( "%s\t%i", temp->dict.GetString( "shop_name", temp->GetName() ), temp->dict.GetInt( "cost", "100" ) ) );
									gui->SetStateString( va( "otherIndex_item_%i", c), va( "%i", temp->Index() ) );
									c++;
									temp = NULL;
								}
							}
						...
						gui->SetStateBool( "networkState", "1" );
