...
enum {
	...
	GAME_RELIABLE_MESSAGE_CHANGECLASS // MyDevil Added (Silly Rabbit Player Classes)
};
...
public:
	...
	const idDeclEntityDef * FindEntityDefByIndex( const int num, bool makeDefault ) const; // SnoopJeDi
...
private:
    void                    SpawnPlayerAs( int clientNum, const char* classname);
 
public:
    bool                    ChangeClassNow( int clientNum, const char* classname);
