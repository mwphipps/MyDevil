...
// MyDevil Changed
	void					AddAIKill( int addexp, int addgold ); 
	int                     giveExp( int addExp );
    void 		            CheckExp( void );	// Mydevil
    int                     GetLevel( void ); // SnoopJedi  Mydevil
//     
...
// MyDevil Added (Silly Rabbit Player Classes)
   idStr nextClass; 
public:
   bool                     CanChangeClassTo( const char* classname );
   void                     SetNextClass( const char* classname ); 
   const char*              GetNextClass( void );   
};
