...
void idGameLocal::LocalMapRestart( ) {
  ...
  // since it effects deathmatch maps also
	...
	MapPopulate();
	mpGame.SetUpShops();  // SnoopJedi - Mydevil added
...
idGameLocal::SpawnPlayer
  SillyRabbit: I have split SpawnPlayer into two functions, SpawnPlayer and SpawnPlayerAs.
  because the SpawnPlayer signature for SpawnPlayer can't be changed
============
*/
void idGameLocal::SpawnPlayer( int clientNum ) {
        SpawnPlayerAs( clientNum, NULL );
}

/*
===========
idGameLocal::SpawnPlayerAs
  SillyRabbit: this is a helper function for SpawnPlayer and ChangeClassNow
    it is identical to the old SpawnPlayer except that it takes
    a second arg 'classname'
===========
*/
void idGameLocal::SpawnPlayerAs( int clientNum, const char* classname ) {
        idEntity        *ent;
        idDict          args;

        // if classname is null, spawn the player as the default class
        if ( !classname || !*classname ) {
                classname = ( isMultiplayer ? "player_doommarine" : "player_doommarine_mp" );
        }

        // they can connect
        Printf( "Spawning player %i as '%s'\n", clientNum, classname ); // bit of debuging here
...
bool idGameLocal::ChangeClassNow( int clientNum, const char* classname ) {
        idPlayer *player;

        // overzealous sanity checks
        assert( !isClient );
        assert( classname != NULL );
        assert( clientNum >= 0 && clientNum < MAX_CLIENTS );
        player = GetClientByNum( clientNum );
        assert( player != NULL );
        assert( player->IsType( idPlayer::Type ) );

   // print a reassuring message to the console...
        Printf( "Changing class of client %d from '%s' to '%s'\n",
                        clientNum,
                        player->spawnArgs.GetString( "classname" ),
                        classname );

        // okay lets get down to business
        delete player;
        SpawnPlayerAs( clientNum, classname );

        return true;
}
// </SillyRabbit> MULTICLASS
...
/*
================
idGameLocal::FindEntityDefByIndex
================

SnoopJeDi

*/
const idDeclEntityDef *idGameLocal::FindEntityDefByIndex( const int num, bool makeDefault ) const {
      const idDecl *decl = NULL;
      decl = declManager->DeclByIndex( DECL_ENTITYDEF, num, makeDefault );
      if (decl)
...
/*
================
idGameLocal::AllowWeapons - Nicemice: added
================
*/
...
/*
================
idGameLocal::SavePersistantPlayerInfo - Nicemice: added

-we don't use this yet, since it should be gametype dependend
-and somehow i crashed the same after some level changes
-transmitting pdas texts does not work either/untested
================
*/
void idGameLocal::SavePersistantInfo() {
  ...
  for ( int i = 0; i < MAX_CLIENTS; i++ ) {
		if ( entities[ i ] && entities[ i ]->IsType( idPlayer::Type ) ) {
			static_cast< idPlayer * >( entities[ i ] )->SavePersistantInfo();
		}
	}
}
